...
def ktrain( model , xult , yult,splits=5,epochs=15,batch_size=32,random_state=42,pr=False):
    ...
    for train_index, test_index in kf.split(X):
        ...
        # Train the model
        model.fit(xt, yt, validation_data=(xv, yv), epochs=epochs, batch_size=batch_size, verbose=1)
        
        # Evaluate the model
        results = model.evaluate(xv, yv)
        if isinstance(model, LGBMRegressorModel) or isinstance(model, XGBoostRegressorModel2):
    ...
    if not isinstance(model, LGBMRegressorModel) or isinstance(model, XGBoostRegressorModel2):
        try:
...
class XGBoostRegressorModel:
    ...
    def predict(self, X_test):
        ...
        mid =  self.model.predict(X_test).reshape(-1, 1)
        output =  np.clip(mid, 0, 1)
        return output
...
for i in range(9):
    ...
    ktrain(xgb_model, xult, yult,  splits=5, random_state=i )
...
class LGBMRegressorModel:
    def __init__(self, eval_metric='rmse', model_path='lgbm_model.txt', **kwargs):
        self.model = lgb.LGBMRegressor(objective='regression', metric=eval_metric, **kwargs)
        self.model_path = model_path
        self.THRESHOLD = 0.5    
        self.booster = self.load_model()

    def fit(self, X_train, y_train, verbose=0, **kwargs):
        self.model.fit(X_train, y_train, init_model=self.booster, )
        self.save_model()

    def predict(self, X_test):
        return self.model.predict(X_test).reshape(-1, 1)

    def f1max(self, iter=0,epochs=0,splits=0):
        pass
        # print(self.summary())
        print(f'iter: {iter}, epochs: {epochs}, splits: {splits}')
...
for i in range(18):
    model_path = ""
    if i % 2 == 0:
        model_path = "lgbm_modelrrr.txt"
        lgbm_model = LGBMRegressorModel(eval_metric='rmse', model_path=model_path, **best_params)
    else:
        model_path = "lgbm_modelmmm.txt"
        lgbm_model = LGBMRegressorModel(eval_metric='mae', model_path=model_path, **{**best_params, 'max_depth': -1})
    ktrain(lgbm_model, xult, yult, splits=5, random_state=i)
...
class XGBoostClassifierModel:
    ...
    def evaluate(self, X_test, y_test):
        ...
        print(cm)
        loss = -1 
        # Placeholder, as XGBoost does not directly provide loss in this context
...
class AutoencoderModel:
    def __init__(self, input_dim, encoding_dim=512, model_path='autoencoder_model.keras'):
        ...
        if os.path.exists('best.keras'):
            print(colored('Loading model from best.keras...', 'yellow'))
            self.autoencoder = load_model('best.keras')
        else:
            print(colored('Loading model from build model...', 'red'))
    ...
    def fit(self, X_train, y_train, epochs=10, verbose=0, batch_size=32, validation_split=0.3, validation_data=None):
        if validation_data is not None:
            self.autoencoder.fit(X_train, y_train, epochs=epochs, verbose=0, batch_size=batch_size, validation_data=validation_data, validation_split=validation_split)
            return 
...
for i in range(len(dim)):
    for epoch in [1]:
        ...
        # losse, auc, precision, recall, f1, roc_auc = ktrain(automodel, xult, yult, splits=5, epochs=epoch, batch_size=32, random_state=i, model_path='auto.keras')
        # ktrain(automodel, xult, yult, splits=5, epochs=epoch, batch_size=32, random_state=i)
...
# Best parameters found
best_params = {
    'colsample_bynode': 0.5782945539842516,
    'min_child_weight': 1,
    'n_estimators': 501,
    'reg_alpha': 0.0,
    'reg_lambda': 0.0,
...
for i in range(20):
    if i % 2 == 0:
        xgbrf_model = XGBRFClassifierModel(objective='reg:squarederror' , model_path='xgbrf_model.json', **best_params)
    else:
        xgbrf_model = XGBRFClassifierModel(eval_metric='logloss', model_path='xgbrf_model.json', **best_params)
...
# # Assuming losses, aucs, precisions, recalls, f1s, and roc_aucs are lists of lists or flat lists
# metrics = {
#     'Losses': losses,
#     'AUCs': aucs,
