...
def LSTM_train(train_pw_list,train_tc_list,epoch,look_back,LR,lstm_units):
    #归一化
    ...
    attention_mul_left = Flatten(name='flatten0_left')(lstm_out_left)
    ...
    attention_mul_right = Flatten(name='flatten0_right')(lstm_out_right)
    ...
    den = Dense(128, activation='relu', name='dense11')(dp)
    ...
    fname='/home/dajinyu/PycharmProjects/dataprocess180528/1h_lstm_result/LSTM-'+str(lstm_units)+'-'+str(look_back)+'-'+str(epoch)+'.h5'
    model.save(fname)


def LSTM_test(test_pw_list,test_tc_list,lstm_units,look_back,epoch):
    ...
    ename= '/home/dajinyu/PycharmProjects/dataprocess180528/1h_lstm_result/LSTM-' + str(lstm_units) + '-' + str(look_back) + '-' + str(epoch) + '.xls'
    save_result(testY, testPredict, ename)
    print('finish')

def main():
    LSTM_train(train_pw_list,train_tc_list,epoch=16,look_back=6,LR=0.0005,lstm_units=32)
    LSTM_test(test_pw_list,test_tc_list,lstm_units=32,look_back=6,epoch=16)
